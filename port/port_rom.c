...
/* ------------------------------------------------------------------ */
#ifdef __vita__
#define ROM_EXTRACT_DIR VITA_ROMDATA_DIR
#else
...
void* Port_ResolveEwramPtr(u32 gba_addr) {
    /* --- gMapBottom (MapLayer at GBA 0x02025EB0) --- */
    {
        ...
        if (gba_addr >= GBA_BASE && gba_addr < GBA_BASE + GBA_SIZE) {
             ...
             * All fields after bgSettings are shifted by +4 on 64-bit PC.
             * On 32-bit (Vita), pointers are 4 bytes, so no shift.
             */
...
static int GetExeDir(char* out, size_t n) {
    ...
    out[0] = '\0';
#ifdef __vita__
    /* Sem conceito de exe dir no Vita. TryOpenRom vai direto ao Pass 2. */
    return 0;
#elif defined(_WIN32)
    char buf[MAX_PATH];
